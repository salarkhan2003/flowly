...
export const useAuthStore = create<AppState>((set, get) => ({
  ...
  resetApp: async () => {
    ...
    await storage.remove('update_banner_snooze_until');
    await storage.remove('update_banner_snooze_version');
    await storage.remove('check_updates_banner_snooze_until');
